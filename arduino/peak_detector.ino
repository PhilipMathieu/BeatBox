...
void update_noise(int i, int voltage){
    ...
    THRESHOLD[i] = (THRESHOLD[i] + K*sqrt(NOISE_SQUARES[i] - NOISE_SUM[i])/NOISE_CNT[i])/2;
}
...
void update_peak(int i, int voltage){
    ...
    else if(PEAK_CNT[i] >= FILTER && voltage < (PEAK_SUM[i]/PEAK_CNT[i])){
